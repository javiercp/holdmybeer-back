...
class KegCreateTest(APITestCase):
    def setUp(self):
        ...
        self.data = {'pintprice': 4.5, 'canyaprice':2.5, 'beer':self.beer.id, 'kegtype':self.kegtype.id}
...
class KegReadTest(APITestCase): 
    ...
    def test_read_keg_ex(self):
        """
        Ensure we can read a keg object with all data.
        """
        url = reverse('keg-detail-ex', args=[self.keg.id])
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
